...
export const getMailsScript = (mailsData: any) => `
  ...
  const settingsMailAccountId = ${vscode.workspace.getConfiguration().get('biyom-code-manager.mailId') ?? 1}; 
  ...
  const fillMail = (mail, mailId, tabIndex) => {
    ...
    const templateSubject = mail.templates[mailId].subject.length ? mail.templates[mailId].subject : ' ';

    document.querySelector('.window.activedWindow .tabContent[index="' + tabIndex + '"] input[name*="mailDisplayName"]').value = templateName;
    document.querySelector('.window.activedWindow .tabContent[index="' + tabIndex + '"] input[name*="subject"]').value = templateSubject;

    const editors = document.querySelectorAll('.window.activedWindow .tabContent[index="' + tabIndex + '"] .ace_editor');
    editors.forEach((editor)=>{
      ...
      }else if(editorClassList.some(className => className.includes('body'))){
        // setBody
  ...
  const editMailCallback = (mutationList, observer) => {
    const mailWindowElement = mutationList.some(ele => ele.target.matches('.windowFooter'));  
    let needsToClose = false;

    if(mailWindowElement){
      if(!document.getElementById('-lucee-err')){
        const languages = document.querySelector('form input[type="hidden"][name="languages"]')?.value?.split(',');
        const languageTabsContainerId = document.querySelector('.languageTabsContainer').id;
        const mailType = document.querySelector('form input[type="hidden"][name="type"]')?.value;
        
        // selectMailAccountId 
        const mailAccountSelector = document.querySelector('.window.activedWindow select[name="mailAccountId"]');
        if(mailAccountSelector.options.length !== 1){
          ...
          if(languages && mailType){
            ...
            document.querySelector('.window.activedWindow .footerButtons input[type="submit"]').click();
          }
        }else{
          console.log(
      ...
      const responseMessageTarget = mutationList.find(ele => ele.target.matches('.responseMessageTarget.okMessage'));
      if(responseMessageTarget && responseMessageTarget.target.style.visibility === 'visible'){
    ...
    if(needsToClose){
      document.querySelector('.window.activedWindow .titleBar .closeButton').click();
      if(defaultMails.length){
        ...
        console.log('%cVScode Emilio Generator ☕\\nEnjoy! 🌼', 
